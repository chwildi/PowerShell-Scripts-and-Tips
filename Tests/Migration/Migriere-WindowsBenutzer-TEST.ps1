...
[CmdletBinding()]
param(
    [Parameter()][string]$ScriptPath = (Join-Path $PSScriptRoot '..\..\Scripts\Migration\Migriere-WindowsBenutzer.ps1'),
    [Parameter()][switch]$RequirePSScriptAnalyzer
)
...
# CONFIGURATION BLOCK
# ============================================================================
$Configuration = [ordered]@{ScriptPath=$ScriptPath;RequirePSScriptAnalyzer=[bool]$RequirePSScriptAnalyzer;RequiredVersion='7.0';RequiredScriptVersion='2.0.0.0'}
$results = [System.Collections.Generic.List[object]]::new()
function Add-TestResult {[CmdletBinding()]param([Parameter(Mandatory)][string]$Name,[Parameter(Mandatory)][bool]$Passed,[Parameter(Mandatory)][string]$Detail);$results.Add([pscustomobject]@{Test=$Name;Passed=$Passed;Detail=$Detail})}
...
if($analyzer){Import-Module PSScriptAnalyzer -ErrorAction Stop;$issues=@(Invoke-ScriptAnalyzer -Path $Configuration.ScriptPath -Severity @('Error','Warning'));Add-TestResult -Name 'PSScriptAnalyzer' -Passed ($issues.Count -eq 0) -Detail $(if($issues.Count -eq 0){'No Error/Warning findings'}else{($issues|ForEach-Object{"[$($_.RuleName)] line $($_.Line): $($_.Message)"}) -join ' | '})}else{Add-TestResult -Name 'PSScriptAnalyzer available' -Passed (-not $Configuration.RequirePSScriptAnalyzer) -Detail 'Install-Module PSScriptAnalyzer -Scope CurrentUser'}
$results|Format-Table -AutoSize
$failed=@($results|Where-Object{-not $_.Passed})
if($failed.Count -gt 0){throw "$($failed.Count) test(s) failed: $($failed.Test -join ', ')"}
Write-Information -MessageData "`nAll tests passed." -InformationAction Continue
